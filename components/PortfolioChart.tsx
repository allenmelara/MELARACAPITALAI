...
export default function PortfolioChart({ history }: { history: PortfolioHistoryPoint[] }) {
  if (history.length < 2) {
    return (
      <p className="disclaimer">
        Performance history starts building from today — check back after a few days to see a trend line.
      </p>
    );
  }
  ...
  return (
    <ResponsiveContainer width="100%" height={240}>
      <LineChart data={history} margin={{ top: 8, right: 8, left: -16, bottom: 0 }}>
        ...
        />
        <Line type="monotone" dataKey="value" name="Portfolio value" stroke={ACCENT} strokeWidth={2} dot={false} />
      </LineChart>
    </ResponsiveContainer>
  );
}
